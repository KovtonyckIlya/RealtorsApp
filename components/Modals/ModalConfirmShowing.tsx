...
import { confirmShowing } from '@actions/showingsActions';
...
function ModalConfirmShowing({ chooseDate, showing, onPressClose }: modalTypes) {
    const [confirmTime, setConfirmTime] = useState<number>(NaN);
    const [confirmDate, setConfirmDate] = useState<number>(NaN);
    const dispatch = useDispatch();

    const getDate = (): Moment => {
        const day = moment(confirmDate).format('YYYY-MM-DD');
        const hoursTime = moment(confirmTime).format('HH:mm:ss');
        return moment(`${day} ${hoursTime}`, 'YYYY-MM-DD HH:mm:ss');
    };

    const onPress = () => {
        dispatch(
            confirmShowing(getDate(), showing.shortId)
        );
        onPressClose(false);
    };
        ...
        >
            <View style={styles.container}>
                <View style={styles.headerView}>
                    <Text style={styles.titleText}>CONFIRM TIME</Text>
                    <View>
                        <TouchableOpacity
                ...
                <ScrollView>
                    ...
                    <ScrollHours maxSelects={1} setValueSelect={(arr) => {
                        console.log('select scroll hours', arr)
                        setConfirmTime(arr[0])
                    }} />
                ...
                <Footer>
                    <DarkButtonFilled disabled={!confirmTime || !confirmDate} onPress={onPress} title="Confirm" />
                </Footer>
            </View>
        </Modal>
    );
...
export default ModalConfirmShowing;
